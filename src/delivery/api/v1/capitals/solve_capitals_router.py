...
from src.use_cases.solve_capitals_command_handler import (
    SolveCapitalsCommand,
    SolveCapitalsCommandHandler,
)
...
@solve_capitals_router.post(
    "/api/v1/capitals/solve",
    response_model=SolveCapitalsResponse,
    response_model_exclude_none=True,
)
def solve_capitals(
    solve_capitals_request: SolveCapitalsRequest,
    handler: SolveCapitalsCommandHandler = Depends(_solve_capitals_command_handler),
    ...
    try:
        ...
        raise HTTPException(
            status_code=BAD_REQUEST, detail=f"The country is not valid"
        ) from exception
