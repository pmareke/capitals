...
from src.delivery.api.v1.countries.solve_countries_response import SolveCountriesResponse

from src.domain.command_handler import CommandHandler
from src.use_cases.solve_countries_command_handler import SolveCountriesCommand, SolveCountriesCommandHandler
...
@solve_countries_router.post("/api/v1/countries/solve",
                            response_model=SolveCountriesResponse,
                            response_model_exclude_none=True)
def solve_countries(
    solve_countries_request: SolveCountriesRequest,
    handler: SolveCountriesCommandHandler = Depends(
        _solve_countries_command_handler)
    ...
    try:
        ...
        raise HTTPException(status_code=HTTPStatus.BAD_REQUEST,
                            detail=f"The fk is not valid") from exception
