...
class SqlCountriesRepository(CountriesRepository):
    ...
    def _insert_countries_into_db(self, cursor: Cursor, countries: List[Country]) -> None:
        values = [(country.name, country.capital, country.flag.image, country.flag.alt) for country in countries]
        cursor.executemany(
            f"INSERT INTO countries(name, capital, flag_url, flag_alt) VALUES (?, ?, ?, ?)", values
            )
